...
In case you require an older or simply a fixed version of Kubectl, the following tagged images are also made available from the Docker hub. In each of these images the version is fixed and won't be changed in the future. The corresponding embedded Kubectl versions were released with the indicated versions of k8s:
